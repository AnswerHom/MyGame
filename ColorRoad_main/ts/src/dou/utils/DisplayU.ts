...
module dou.utils{
	...
	export class DisplayU{
		...
		public static MASK_TYPE_INIT	:number = 1;	//初始化遮罩
		...
		public static initMask():void{
			DisplayU.MASK_IMG_TOP  = new LImage(Path.ui+"tongyon/image_mask_top.png");
			DisplayU.MASK_IMG_BOTTOM = new LImage(Path.ui+"tongyon/image_mask_bottom.png");
			DisplayU.MASK_IMG_MIDDLE = new LImage(Path.ui+"tongyon/image_mask_middle.png");
			DisplayU.MASK_IMG_LEFT  = new LImage(Path.ui+"tongyon/image_mask_left.png");
			DisplayU.MASK_IMG_RIGHT  = new LImage(Path.ui+"tongyon/image_mask_right.png");
		...
		public static onScrollChange(target:any,type:number=DisplayU.MASK_TYPE_NORMAL):void{
			...
			if(scroll.min == scroll.max){//不足一屏
				if(type == DisplayU.MASK_TYPE_INIT) this.drawMaskByType(target,DisplayU.MASK_KIND_ALL);
				return;
			}
		...
		public static onHScrollChange(target:any,type:number=DisplayU.MASK_TYPE_NORMAL):void{
			...
			if(!scroll || scroll.isVertical) return;
			if(scroll.min == scroll.max){//不足一屏
				if(type == DisplayU.MASK_TYPE_INIT) this.drawHMaskByType(target,DisplayU.MASK_KIND_ALL);
				return;
			}
