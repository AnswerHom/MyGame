...
module dou.scene{
	export class Effect{
		...
		public set drawHorizonal(v:boolean)
		{
		...
		public realPosX:number;
		public realPosY:number;
		...
		protected drawCalculate(): void {
			//获取绘制相关
			// let params: Array<any> = 0//AvatarData.IMAGE_TABLE[this._toward];
			//设置方向
			this._drawDirect = 0;
			//设置是否镜像
			this._drawHorizonal = false;
		}
		...
		protected updateTransform(camera: Camera):void{
			...
			this._drawMix = this._tempMix;
			this._drawMix.identity();
			if(this._drawHorizonal)
			{
				
				this._drawMix.a = -1;
				this._drawMix.tx = this._drawX * 2 - this._regX;
				this._drawMix.ty = this._regY;
			}
			...
			if(this.roation != 0)
			{
				...
				this._drawMix.rotate(this.roation);
				this._drawMix.translate(this._drawX, this._drawY);
				this._drawX = 0;
				this._drawY = 0;
			}
