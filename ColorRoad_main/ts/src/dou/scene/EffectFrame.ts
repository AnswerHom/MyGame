...
module dou.scene {
	export class EffectFrame extends Effect implements IPoolsObject {
		...
		constructor() {
			...
			this.setAssetPath(Path.scene_sf);
		}
		...
		// 设置数据
		setData(data: string, fps: number = 30,dataFix:string="",start:number=10000): void {
		...
		setFrames(img: string, start: number = 0): void {
			let frames = Path.getSeqFrames(img, this._total, start);
		...
		protected updateTransform(camera: Camera): void {
			...
			this.realPosX = this._drawX;
			this.realPosY = this._drawY;
			if (this._scale == 1) {
