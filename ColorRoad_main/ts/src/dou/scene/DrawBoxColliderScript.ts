module dou.scene{
    export class DrawBoxColliderScript extends Laya.Script {
        ...
        public _postRenderUpdate(state:Laya.RenderState):void {
            let any1 = ((this._owner as Laya.Sprite3D).getComponentByType(Laya.BoxCollider) as Laya.BoxCollider);
            let boundBox = null;
            if(any1) boundBox = any1.boundBox;
            else
            {
                let any2 = ((this._owner as Laya.Sprite3D).getComponentByType(Laya.MeshCollider) as Laya.MeshCollider);
                if(any2)
                    boundBox = any2._boundBox
            }
            if(!boundBox)return;
            var obb:Laya.OrientedBoundBox = boundBox;
