...
(function()
{
	...
	LayaRender.onRemoveSprite = function(args)
	{
		...
		var container = args.object.spriteContainer;
	...
	LayaRender.body = function(render, body)
	{
		...
		if (bodyRender.sprite && bodyRender.sprite.texture)
		{
			var spriteId = 'b-' + body.id,
				...
				container = render.container,
				spriteContainer = body.spriteContainer;
			if(!spriteContainer)
			{
				spriteContainer = body.spriteContainer = new Laya.Sprite();
			}

			
			// 如果sprite不存在，则初始化一个
			if (!sprite || body.render.reDraw)
			{
				sprite = body.layaSprite = _createBodySprite(render, body, sprite);
				body.render.reDraw = false;
			}
			...
			if (!container.contains(spriteContainer))
				container.addChild(spriteContainer);

			// 更新sprite位置
			spriteContainer.x = body.position.x;
			spriteContainer.y = body.position.y;
			var angle = Math.floor(body.angle * 180 / Math.PI)
			sprite.rotation = angle % 2 == 0 ? angle : angle + 1 ;
			sprite.scaleX = bodyRender.sprite.xScale || 1;
			sprite.scaleY = bodyRender.sprite.yScale || 1;

			//直接给加个文本
			if(body.text && body.text.parent != spriteContainer)
			{
				spriteContainer.addChild(body.text);
				// body.text.rotation = -body.angle * 180 / Math.PI;
			}
		...
		else // 没有纹理的body
		{
			...
			if (!sprite || body.render.reDraw)
			{
				sprite = body.layaSprite = _createBodyPrimitive(sprite, render, body);
				sprite.cacheAsBitmap = true;
				sprite.scaleX = body.xScale || 1;
				sprite.scaleY = body.yScale || 1;
				sprite.initialAngle = body.angle;
				body.render.reDraw = false;

				// var bodyRender = body.render,
				// texturePath = bodyRender.texture;
				// var sprite11 = body.render.xxx || new Laya.Image(texturePath);
				// body.render.xxx = sprite11;
				// sprite11.skin = texturePath;
				// // sprite.loadImage(texturePath);
				// sprite11.pivotX = body.render.sprite.xOffset;
				// sprite11.pivotY = body.render.sprite.yOffset;
			}
			...
			// if (!container.contains(body.render.xxx))
			// 	container.addChild(body.render.xxx);
			// 更新sprite位置
			sprite.x = body.position.x;
			sprite.y = body.position.y;

			// body.render.xxx.x = body.position.x;
			// body.render.xxx.y = body.position.y;

			// body.render.xxx.rotation = (body.angle ) * 180 / Math.PI;
			sprite.rotation = (body.angle - sprite.initialAngle) * 180 / Math.PI;
			//直接给加个文本
			if(body.text && body.text.parent != sprite)
			{
	 ...
	 * @return {void}
	 */
	var _createBodySprite = function(render, body, sprite)
	{
		var bodyRender = body.render,
			texturePath = bodyRender.sprite.texture,
			sprite = sprite || new Laya.Image(texturePath);
        sprite.skin = texturePath
		// sprite.loadImage(texturePath);
	 ...
	 * @return {void}
	 */
	var _createBodyPrimitive = function(sprite, render, body)
	{
		var bodyRender = body.render,
			options = render.options,
			sprite = sprite || new Laya.Sprite(),
			fillStyle, strokeStyle, lineWidth,
			part, points = [];

		var primitive = sprite.graphics;
		primitive.clear();
		// primitive.drawPoly(0,0,[0,0,300,0,0,300,0,0], "#FF")
		// primitive.dr
		// 处理 compound parts
		for (var k = body.parts.length > 1 ? 1 : 0; k < body.parts.length; k++)
		{
			...
			points.push(Math.floor(part.vertices[0].x - body.position.x), Math.floor(part.vertices[0].y - body.position.y));

			for (var j = 1; j < part.vertices.length; j++)
			{
				points.push(Math.floor(part.vertices[j].x - body.position.x), Math.floor(part.vertices[j].y - body.position.y));
			}

			points.push(Math.floor(part.vertices[0].x - body.position.x), Math.floor(part.vertices[0].y - body.position.y));
			if(points.length >= 54)
			{
				primitive.drawCircle(0, 0, body.circleRadius, fillStyle, fillStyle, lineWidth);
				return sprite;
			}
